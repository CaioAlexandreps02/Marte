...
O mapa e definido em mapa.json (tamanho, canto sudoeste, agua, inicio) e as edicoes a mao
(platos) em edicoes.json. O processamento e feito em faixas de 1 km para caber na memoria.
...
from scipy.ndimage import affine_transform, distance_transform_edt, gaussian_filter, spline_filter, uniform_filter
...
def aplicar_platos(z, d, platos, x_origem=0.0, z_origem=0.0):
    """Nivela areas (grade com linha 0 = SUL). Dentro do raio a altura vira `altura_m` mais uma fracao
    (`detalhe`) do microrrelevo original; na transicao mistura com o relevo real (smoothstep).
    `irregularidade` deforma a borda com senoides de fase fixa (`semente`).
    ...
    for p in platos:
        ...
        rng = np.random.default_rng(p.get("semente", 0))
        forma = sum(rng.uniform(0.5, 1) / k * np.sin(k * ang + rng.uniform(0, 2 * np.pi)) for k in range(2, 9))
        dist = dist / (1 + irr * forma / 1.2)
        t = np.clip((alcance - dist) / p["transicao_m"], 0, 1)
        w = t * t * (3 - 2 * t)
        detalhe = win - gaussian_filter(win, 20 / d)
        plano = p["altura_m"] + p["detalhe"] * detalhe
        z[r0:r1, c0:c1] = (win * (1 - w) + plano * w).astype(np.float32)
        print(f"plato '{p['nome']}': raio {p['raio_m']} m + transicao {p['transicao_m']} m em {p['altura_m']} m")
...
def aplicar_canyons(z, d, canyons, cache, x_origem=0.0, z_origem=0.0):
    ...
    for cy in canyons:
        ...
        src_npy = cache / f"stamp_{cy['nome']}.npy"
        if not src_npy.exists():
            with rasterio.open("/vsicurl/" + cy["fonte_url"]) as s:
                ...
                res = abs(s.res[0])
            np.save(src_npy, a); (cache / f"stamp_{cy['nome']}.json").write_text(json.dumps({"res": res}))
        src = np.load(src_npy)
        res = json.loads((cache / f"stamp_{cy['nome']}.json").read_text())["res"]
        ...
        c0, c1 = max(0, int(xa / d)), min(n_col, int(xb / d) + 1)
        yy, xx = np.mgrid[r0:r1, c0:c1]
        rel = np.stack([xx * d - de[0], yy * d - de[1]], -1)
        ud, vd = rel @ ed, rel @ pd                           # ao longo / de lado (m), vd > 0 = esquerda
        del rel, yy, xx
        # ponto correspondente na origem (comprimento esticado, largura igual)
        us = u0 + np.clip(ud, 0, comp) * (comp_src / comp)
        px_ = centro[0] + us * eixo[0] + vd * perp[0]
        py_ = centro[1] + us * eixo[1] + vd * perp[1]
        from scipy.ndimage import map_coordinates
        cheio = np.where(valido, src, planalto)
        s_amostra = map_coordinates(cheio, [-py_ / res, px_ / res], order=1, mode="nearest").astype(np.float32)
        # planalto local de cada seccao (faixa lateral) -> so a diferenca entra no terreno
        ...
        delta = (s_amostra - ref[bins]) * k
        t_ponta = np.clip(np.minimum(ud, comp - ud) / (cy["afinar_pontas_km"] * 1000), 0, 1)
        t_lado = np.clip((meia - np.abs(vd)) / (meia * 0.3), 0, 1)
        w = (t_ponta * t_ponta * (3 - 2 * t_ponta)) * (t_lado * t_lado * (3 - 2 * t_lado))
        z[r0:r1, c0:c1] = (np.array(z[r0:r1, c0:c1]) + delta * w).astype(np.float32)
...
def main():
    ...
    aplicar_suavizacoes(z, d, edicoes.get("suavizacoes", []), ox, oz)
    aplicar_canyons(z, d, edicoes.get("canyons", []), a.cache, ox, oz)
    aplicar_platos(z, d, edicoes.get("platos", []), ox, oz)
    z.flush()
    ...
    # ---- ponto de inicio: fundo plano do lago seco, perto do alvo (grade ~4 m, linha 0 = sul) ----
    z4, d4 = np.array(z[::4, ::4]), d * 4
    zs4 = gaussian_filter(z4, 2); gy, gx = np.gradient(zs4, d4)
    ...
    yy, xx = np.mgrid[0:z4.shape[0], 0:z4.shape[1]]; X, Y = xx * d4 / 1000, yy * d4 / 1000
    alvo_x, alvo_y = cfg["inicio_alvo_km"][0] + g["desloc_x"] / 1000, cfg["inicio_alvo_km"][1] + g["desloc_z"] / 1000
    i = np.unravel_index(np.argmin(np.where(baixo, ms + 0.4 * np.hypot(X - alvo_x, Y - alvo_y), 1e9)), z4.shape)
    ...
    ax.set_title(f"Jezero {TX}x{TZ} km - {hmin} a {hmax} m | azul: agua em {nivel_agua:.0f} m | vermelho: area jogavel\n"
                 f"estrela: inicio | quadrado: plato | HiRISE 1 m em {100 * frac_hirise:.0f}% do mapa", fontsize=11)
    ...
    meta = {
        ...
        "inicio_jogador": {"x_m": round(sx, 1), "z_m": round(sz, 1), "tile": [int(sx // 1000), int(sz // 1000)],
                           ...
                           "criterio": f"menor inclinacao media em 300 m, fundo do lago entre agua+2 m e -2512 m, perto de ({alvo_x:.1f}; {alvo_y:.1f}) km do mapa"},
